...
async function fetchActivities(key) {
    try {
        ...
        if (response.timeline[key]) {
            console.log(response.timeline[key])
            try {
                validateMinCoverage(response.timeline[key].min_coverage);
            } catch (error) {
        ...
        const timeline = response.timeline[key];
        ...
        if (DEBUG_MODE) {
            ...
            console.log('All available timelines in activities.json:', Object.keys(response));
            console.log('Full timeline data:', response);
            console.log('Initialized timelines:', Array.from(window.timelineManager.initialized));
        }

        return response.timeline[key].categories;
    } catch (error) {
            ...
            category.activities.forEach(activity => {
                ...
                activityButton.addEventListener('click', () => {
                    ...
                    if (activity.name.toLowerCase().includes('other not listed (enter)') ||
                        ...
                        activity.name.toLowerCase().includes('(especifique)')
                    ) {
                        // Show custom activity modal
                        const customActivityModal = document.getElementById('customActivityModal');
                        const customActivityInput = document.getElementById('customActivityInput');
...
async function getActivities() {
    try {
        const res = localStorage.getItem('Activities')
